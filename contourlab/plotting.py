...
def stack_contours_in_z(
    ...
    """
    Take a list of 2D contour plots (alredy generated by plot_contour)
    and stack them along Z axis in 3D.

    Args:
        contour_results : list of matplotlib.contour.QuadContourSet
            Typically the 'contour' objects returned by plot_contour(...).
        figsize : tuple
            Figure size.
        z_gap : float or None
            Vertical spacing between successive slices. If None and z_offsets
            is None, a default is computed from XY extents.
        z_offers : sequence of float or None
            Absolute Z positions for each contour set. If provided, takes precedent
            over z_gap. Must have len == len(contours_list).
        elev, azim : float
            view angles for the 3D plot.


    Returns:
        dict with keys: {'fig', 'ax', 'z_offsets'}
    """

    fig = plt.figure(figsize=figsize)
    ax = fig.add_subplot(111, projection="3d")

    # --- global XY limits across all contours ----------------------------
    gx_min, gx_max = float("inf"), -float("inf")
    gy_min, gy_max = float("inf"), -float("inf")

    for cs in contours_list:
        # Guard: empty contour set
    ...
    # --- Draw each contour as 3D line segements at its z offset -----------
    ...
    if offsets:
        zmin = min(offsets) - 0.5 * (
            offsets[1] - offsets[0] if len(offsets) > 1 else 1.0
        )
        zmax = max(offsets) + 0.5 * (
            offsets[1] - offsets[0] if len(offsets) > 1 else 1.0
        )
    ...
    ax.set_title("Stacked 2D contours in 3D")
    plt.tight_layout()
